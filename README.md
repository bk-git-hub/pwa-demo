...
알림 권한을 한 번 거부하면 같은 사이트에서 native permission prompt를 계속 다시 띄울 수 없습니다. 사용자가 브라우저 또는 iOS 알림 설정에서 직접 허용으로 바꾼 뒤 앱에서 권한 상태를 다시 확인해야 합니다.

위치 지도는 OpenStreetMap embed를 사용하므로 별도 API 키가 필요 없습니다. 많은 사용자에게 제공하는 실제 서비스라면 지도 제공자의 이용 약관, 트래픽 제한, 개인정보 처리 기준을 따로 확인해야 합니다.
